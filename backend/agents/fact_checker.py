...
class FactChecker:
    ...
    async def verify(self, claim: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Verifying ({language}): {claim_text[:50]}...")

        web_chunks = self.web_searcher.search_and_index(claim_text, num_results=5, language=language)
        evidence_chunks = web_chunks if web_chunks else self.retriever.retrieve(claim_text, top_k=config.RAG_TOP_K)
        ...
        prompt = build_fact_check_prompt(claim_text, evidence_chunks, language=language)
